...
def GetAddonInfo():
    ...
    addon_info["name"] = addon_info["addon"].getAddonInfo("name")
...
def OpenURL(url):
    with requests.Session() as session:
        ...
        result = get_url(url, session=session, headers=headers, result_callback=result_callback, life=datetime.timedelta(hours=1))
        # log("OpenURL, url = %s, result = %s" % (url, result))
        return result
...
def KidsMode():
    ...
    if old_password == password:
        ...
        ADDON.setSetting('kids_password', encoded_password)
    quit()
...
def log(txt, level=xbmc.LOGDEBUG):
    message = ''.join([addoninfo['name'], ' : ', txt])
...
def get_url(
    ...
    **kwargs
    ...
    log(f"get_url URL: {url}, Headers: {headers}")
    ...
    cache_enabled = life.total_seconds() > 0
    cachekey = f"{addoninfo['name']}.getURL.{url}.{params}.{headers}"

    # Try to get response from cache
    cacheresponse = cache.get(cachekey) if cache_enabled else None
    if cacheresponse:
        log(f"Cache hit for URL: {url}, Headers: {headers}")
    ...
    try:
        ...
        if session:
            ...
            r = requests.get(method, url, params=params, headers=headers, **kwargs)
        
        r.raise_for_status()

        # Process response using result_callback or default to text
        cacheresponse = result_callback(r) if result_callback else r.text

        # Close the response and update the cache
        r.close()
        cache.set(cachekey, cacheresponse, expiration=life)

    except Exception as e:
        log(f"get_url failed! {e}", xbmc.LOGERROR)
        dialog = xbmcgui.Dialog()
        dialog.ok(translation(30400), f"{e}")
        sys.exit(1)

    # Update cookies if there are redirections
    try:
        if r.history:
            for cookie in r.cookies:
                cookie_jar.set_cookie(cookie)
            # Set ignore_discard to overcome the issue of not having a session
            # as the cookie_jar is reinitialized for each action.
            cookie_jar.save(ignore_discard=True)
    except Exception:
        pass
