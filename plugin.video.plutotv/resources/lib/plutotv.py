...
KIDS_MODE     = REAL_SETTINGS.getSettingBool('Kids_Mode')
RATING        = REAL_SETTINGS.getSetting('Rating').strip()
WHITELIST_ITEMS             = REAL_SETTINGS.getSetting('Whitelist_Items').strip()
BLACKLIST_CATEGORIES        = REAL_SETTINGS.getSetting('Blacklist_Categories').strip()
BLACKLIST_ITEMS             = REAL_SETTINGS.getSetting('Blacklist_Items').strip()
RATING                      = [item.strip() for item in RATING.split(',') if item.strip()] if RATING else []  # When more than one elements, keep the "''" element
WHITELIST_ITEMS             = [item.strip() for item in WHITELIST_ITEMS.split(',') if item.strip()] if WHITELIST_ITEMS else []  # When more than one elements, keep the "''" element
BLACKLIST_CATEGORIES        = [item.strip() for item in BLACKLIST_CATEGORIES.split(',') if item.strip()] if BLACKLIST_CATEGORIES else []  # When more than one elements, keep the "''" element
BLACKLIST_ITEMS             = [item.strip() for item in BLACKLIST_ITEMS.split(',') if item.strip()] if BLACKLIST_ITEMS else []  # When more than one elements, keep the "''" element
GUIDE_URL     = 'https://service-channels.clusters.pluto.tv/v1/guide?start=%s&stop=%s&%s'
MEDIA_VOD     = 'https://service-vod.clusters.pluto.tv/v4/vod/categories/%s/items?offset=10'
...
BASE_VOD      = BASE_API + '/v3/vod/categories?offset=1000&includeItems=true&deviceType=web&%s' if not KIDS_MODE else 'https://service-vod.clusters.pluto.tv/v4/vod/categories?offset=1000&includeItems=true&deviceType=web&%s'
...
def setUUID():
    if REAL_SETTINGS.getSetting("sid1_hex") and REAL_SETTINGS.getSetting("deviceId1_hex") and REAL_SETTINGS.getSetting("clientId1_hex"): return
    REAL_SETTINGS.setSetting("sid1_hex",str(uuid.uuid1().hex))
    REAL_SETTINGS.setSetting("deviceId1_hex",str(uuid.uuid4().hex))
    REAL_SETTINGS.setSetting("clientId1_hex",str(uuid.uuid4().hex))
    # REAL_SETTINGS.setSetting("clientId1_hex",quote(''.join(random.choice(string.ascii_uppercase +
...
def getClientID():
    return REAL_SETTINGS.getSetting("clientId1_hex")
...
class PlutoTV(object):
    def __init__(self, sysARG=sys.argv):
        ...
        self._plutoBootCacheKey = None
        self._ondemandCacheKey = None
    ...
    @property
    def plutoBootCacheKey(self):
        if not self._plutoBootCacheKey:
            self._plutoBootCacheKey = '%s.plutoBoot'%(ADDON_NAME)
        return self._plutoBootCacheKey

    @property
    def ondemandCacheKey(self):
        if not self._ondemandCacheKey:
            self._ondemandCacheKey = '%s.getURL, url = %s.%s.%s'%(ADDON_NAME,BASE_VOD%(LANGUAGE(30022)%(getUUID())),{},self.buildHeader())
        return self._ondemandCacheKey

    def plutoBoot(self):
        # Sample requests: GET https://boot.pluto.tv/v4/start?appName=web&appVersion=7.7.0-18f7ab32608969ea5bcbce8d0e23b9d0e1b24717&deviceVersion=116.0.0&deviceModel=web&deviceMake=chrome&deviceType=web&clientID=a66ed3e5-3ca9-4b71-a578-44ecad752ed0&clientModelNumber=1.0.0&serverSideAds=false&constraints=&drmCapabilities=widevine%3AL3&blockingMode=&clientTime=2023-09-22T17%3A54%3A37.019Z

        cachelife = 28800/2
        cachekey = self.plutoBootCacheKey
        cachevalue = self.cache.get(cachekey)

        if cachevalue:
            clientTime = cachevalue
        else:
            clientTime = datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
            # Make it expired a little early than the response cache
            self.cache.set(cachekey, clientTime, expiration=datetime.timedelta(seconds=cachelife - 3))

        response = self.getURL(
            'https://boot.pluto.tv/v4/start',
            param={
                'appName': 'web',
                'appVersion': '7.7.0-18f7ab32608969ea5bcbce8d0e23b9d0e1b24717',
                'deviceVersion': '116.0.0',
                'deviceModel': 'web',
                'deviceMake': 'chrome',
                'deviceType': 'web',
                ...
                'serverSideAds': 'false',
                'constraints': 'kids-only' if KIDS_MODE else '',
                'drmCapabilities': 'widevine:L3',
                'blockingMode': '',
                'clientTime': clientTime,
            },
            life=datetime.timedelta(seconds=cachelife)
        )

        if DEBUG:
            log('plutoBoot, response=%s'%response)

        # To delete cache while the response is invalid
        if not response.get('sessionToken'):
            self.cache.set(cachekey, clientTime, expiration=datetime.timedelta(seconds=1))

        return response


    def getToken(self):
        boot_data = self.plutoBoot()
        return boot_data['sessionToken']
    ...
    def getURL(self, url, param={}, header={'User-agent': 'Mozilla/5.0 (Windows NT 6.2; rv:24.0) Gecko/20100101 Firefox/24.0'}, life=datetime.timedelta(minutes=15)):
        ...
        cachekey = '%s.getURL, url = %s.%s.%s'%(ADDON_NAME,url,param,header)
    ...
    def buildGuide(self, data):
        ...
        urls      = []
        mtype     = 'video'
        chkids    = channel.get('kidsMode', False)
        if KIDS_MODE and not chkids:
            return

        chid      = channel.get('_id','')
        if len(BLACKLIST_CATEGORIES) > 0 and chid in BLACKLIST_CATEGORIES:
            return
        ...
        if opt in ['channels','categories','ondemand','season']:
            ...
            for item in timelines:
                ...
                if len(BLACKLIST_ITEMS) > 0 and episode.get('_id') in BLACKLIST_ITEMS:
                    continue

                if len(RATING) > 0 and epmpaa not in RATING and (len(WHITELIST_ITEMS) == 0 or episode.get('_id') not in WHITELIST_ITEMS):
                    # log(f'buildGuide.skip, channel={channel["name"]}, item={item}')
                    if type == 'series':
                        return
                    else:
                        continue
                ...
                urls       = urls or tuple([{'type': path["type"], 'url': f'https://cfd-v4-service-channel-stitcher-use1-1.prd.pluto.tv{path["path"]}'} for path in item.get('stitched',{}).get('paths', [])])
                if len(urls) == 0 and type != 'series': continue
                if isinstance(urls, list): 
                    urls  = [url['url'].replace("siloh.pluto.tv", "silo-hybrik.pluto.tv.s3.amazonaws.com").replace("https", "http").replace("master.m3u8", "hls_2400.m3u8") for url in urls if url['type'].lower() == 'hls'][0] # todo select quality
    ...
    def browseOndemand(self, id=None, opt='ondemand'):
        ...
        ondemandData = self.getOndemand()
        categories = ondemandData.get('categories',[])
        if len(categories) == 0:
            self.cache.set(self.ondemandCacheKey, ondemandData, expiration=datetime.timedelta(seconds=1))
            self.cache.set(self.plutoBootCacheKey, self.cache.get(self.plutoBootCacheKey), expiration=datetime.timedelta(seconds=1))

        self.browseGuide(id, opt, data=categories)
        # if opt == 'vod':
            # print(id)
